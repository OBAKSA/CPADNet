...
##########################################################################
## modulation for controllability
...
##########################################################################
##---------- Restormer -----------------------
class CPA_Restormer_SID(nn.Module):
    def __init__(self,
                 inp_channels=3,
                 out_channels=3,
                 dim=48,
        ...
        self.decoder_level1 = nn.ModuleList([
            TransformerBlock(dim=int(dim * 2 ** 1), num_heads=heads[0], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[0])])  # H, W, 2C

        self.refinement = nn.ModuleList([
            TransformerBlock(dim=int(dim * 2 ** 1), num_heads=heads[0], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type) for i in
            range(num_refinement_blocks)])  # H, W, 2C
    ...
    def forward(self, inp_img, param_vector):

        inp_enc_level1 = self.patch_embed(inp_img)
        for block in self.encoder_level1:
            inp_enc_level1 = block(inp_enc_level1, param_vector)
        out_enc_level1 = inp_enc_level1

        inp_enc_level2 = self.down1_2(out_enc_level1)
        for block in self.encoder_level2:
            inp_enc_level2 = block(inp_enc_level2, param_vector)
        out_enc_level2 = inp_enc_level2

        inp_enc_level3 = self.down2_3(out_enc_level2)
        for block in self.encoder_level3:
            inp_enc_level3 = block(inp_enc_level3, param_vector)
        out_enc_level3 = inp_enc_level3

        inp_enc_level4 = self.down3_4(out_enc_level3)
        for block in self.latent:
            inp_enc_level4 = block(inp_enc_level4, param_vector)
        ...
        for block in self.decoder_level3:
            inp_dec_level3 = block(inp_dec_level3, param_vector)
        ...
        for block in self.decoder_level2:
            inp_dec_level2 = block(inp_dec_level2, param_vector)
        out_dec_level2 = inp_dec_level2

        inp_dec_level1 = self.up2_1(out_dec_level2)
        inp_dec_level1 = torch.cat([inp_dec_level1, out_enc_level1], 1)
        for block in self.decoder_level1:
            inp_dec_level1 = block(inp_dec_level1, param_vector)
        out_dec_level1 = inp_dec_level1

        for block in self.refinement:
            out_dec_level1 = block(out_dec_level1, param_vector)
...
class CPA_Restormer_SIDD(nn.Module):
    def __init__(self,
                 inp_channels=3,
                 out_channels=3,
                 dim=48,
        ...
        super(CPA_Restormer_SIDD, self).__init__()
    ...
    def forward(self, inp_img, param, phone):
